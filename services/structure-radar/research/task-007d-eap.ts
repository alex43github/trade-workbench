...
export function validateEapDecision(decision: EapDecision, snapshot: SnapshotLike) {
  ...
  if (decision.decision_bar_close_utc !== snapshotDecisionBar(snapshot)) throw new Error("EAP decision bar identity mismatch");
  if (eapMs < decisionBarMs || eapMs < snapshotDecisionMs) throw new Error("EAP time precedes decision bar");
  ...
  for (const timestamp of decision.causal_evidence.timestamps_utc) {
    if (parseUtc(timestamp, "causal_evidence.timestamp") > eapMs) throw new Error("EAP causal evidence is from the future");
  }
