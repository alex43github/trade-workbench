...
function requestSelection(request: Request) {
  ...
  return { symbol, side: side as "LONG" | "SHORT" | "" };
}
...
export async function POST(request: Request) {
  ...
  try {
    const positionsResult = await getAlexManualPositions();
    if (!positionsResult.connected) return errorResponse(positionsResult.reason ?? "币安手动持仓查询失败", 503);
