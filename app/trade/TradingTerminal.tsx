...
type MarketResponse = { mode: "live"; symbol: string; interval: string; updatedAt: string; warning?: string; bars: MarketBar[] };
...
const defaultStrategy: Strategy = {
  ...
  entryBandPct: 1, entryAtrUpper: 1, entryAtrLower: 1, entryTrigger: "touch_or_close_in_band", sizeMode: "fixed_usdt", sizeValue: 100,
...
export default function TradingTerminal({ initialSymbol }: { initialSymbol: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/trade/indicator-settings?symbol=${encodeURIComponent(symbol)}`, { cache: "no-store" })
      .then((response) => response.ok ? response.json() as Promise<{ settings?: { basis?: "ma" | "ema"; maLength?: number; entryAtrUpper?: number; entryAtrLower?: number; atr?: Partial<IndicatorSettings["atr"]>; vegas?: Partial<IndicatorSettings["vegas"]> } | null }> : Promise.reject(new Error("指标参数读取失败")))
      .then((payload) => {
        if (!active) return;
        ...
        setStrategy((current) => ({ ...current, basis: saved?.basis === "ema" ? "ema" : "ma", maLength: saved?.maLength ?? defaultStrategy.maLength, entryAtrUpper: saved?.entryAtrUpper ?? defaultStrategy.entryAtrUpper, entryAtrLower: saved?.entryAtrLower ?? defaultStrategy.entryAtrLower }));
  ...
  useEffect(() => {
    ...
    indicatorSettingsSaveTimerRef.current = window.setTimeout(() => {
      void fetch("/api/trade/indicator-settings", {
        ...
        body: JSON.stringify({ symbol, basis: strategy.basis, maLength: strategy.maLength, entryAtrUpper: strategy.entryAtrUpper, entryAtrLower: strategy.entryAtrLower, atr: indicators.atr, vegas: indicators.vegas }),
      }).catch(() => undefined);
    }, 350);
    return () => { if (indicatorSettingsSaveTimerRef.current !== null) window.clearTimeout(indicatorSettingsSaveTimerRef.current); };
  }, [symbol, strategy.basis, strategy.maLength, strategy.entryAtrUpper, strategy.entryAtrLower, indicators.atr, indicators.vegas]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (active) {
          setMarketMode("unavailable"); setBars([]);
  ...
  function chooseSymbol(next: string) {
    ...
    setLoading(true); setSymbol(nextSymbol); setSymbolQuery(""); setSymbolSuggestions([]); setSymbolSearchOpen(false);
 ...
 function updateIndicatorLength(key: "ma" | "ema", value: number) {
  ...
  async function closeLive(position: AccountPosition, percent: LiveClosePercent) {
    const clientOrderId = `webMC${crypto.randomUUID().replaceAll("-", "")}`;
      ...
      <div className={styles.appMain}>
        ...
        <section ref={tradeGridRef} className={styles.tradeGrid} style={{ gridTemplateColumns: `minmax(0, 1fr) 10px ${strategyPanelWidth}px` }}>
          <div ref={chartWorkspaceRef} className={styles.chartWorkspace} style={{ "--trade-chart-height": `${chartHeight}px` } as React.CSSProperties}>
            ...
            {indicatorOpen && <IndicatorManager indicators={indicators} basis={strategy.basis} atrUpperMultiplier={strategy.entryAtrUpper} atrLowerMultiplier={strategy.entryAtrLower} setIndicators={setIndicators} updateMaLength={updateMaLength} updateIndicatorLength={updateIndicatorLength} updateVegasLength={updateVegasLength} updateBasis={updateIndicatorBasis} updateAtrBand={(upper, lower) => setStrategy((current) => ({ ...current, entryAtrUpper: upper, entryAtrLower: lower }))} />}
            <TradeChart bars={bars} fills={account.fills} symbol={symbol} theme={resolvedTheme} indicators={indicators} overlays={chartOverlays} indicatorBasis={strategy.basis} atrLength={strategy.maLength} atrUpperMultiplier={strategy.entryAtrUpper} atrLowerMultiplier={strategy.entryAtrLower} />
        ...
        <section className={styles.accountPanel} id="account">
          ...
          {accountTab === "positions" ? <PositionTable positions={account.positions} connected={account.connected} canClose={realTradingStatus.canPlaceOrders} closeDisabledReason={realTradingStatus.detail} onClose={closeLive} onSelectSymbol={chooseSymbol} onAnalyze={(item) => openAnalysisChooser({ symbol: item.symbol, source: "binance", side: item.side, quantity: item.quantity, entryPrice: item.entryPrice, markPrice: item.markPrice, unrealizedPnl: item.unrealizedPnl, leverage: item.leverage, occupiedMargin: item.occupiedMargin })} analysisLoadingSymbol={analysisState === "loading" ? analysisSymbol : ""} /> : <OrderTable orders={[...account.limitOrders, ...account.conditionalOrders]} connected={account.connected} />}
...
function IndicatorManager({ indicators, basis, atrUpperMultiplier, atrLowerMultiplier, setIndicators, updateMaLength, updateIndicatorLength, updateVegasLength, updateBasis, updateAtrBand }: { indicators: IndicatorSettings; basis: "ma" | "ema"; atrUpperMultiplier: number; atrLowerMultiplier: number; setIndicators: React.Dispatch<React.SetStateAction<IndicatorSettings>>; updateMaLength: (value: number) => void; updateIndicatorLength: (key: "ma" | "ema", value: number) => void; updateVegasLength: (key: "fastLength" | "slowLength" | "outerFastLength" | "outerSlowLength", value: number) => void; updateBasis: (basis: "ma" | "ema") => void; updateAtrBand: (upper: number, lower: number) => void }) {
...
function PositionTable({ positions, connected, canClose, closeDisabledReason, onClose, onSelectSymbol, onAnalyze, analysisLoadingSymbol }: { positions: AccountPosition[]; connected: boolean; canClose: boolean; closeDisabledReason: string; onClose: (position: AccountPosition, percent: LiveClosePercent) => Promise<void>; onSelectSymbol: (symbol: string) => void; onAnalyze: (position: AccountPosition) => void; analysisLoadingSymbol: string }) {
  ...
  return <div className={styles.tableWrap}><table><thead><tr><th>分析</th><th>合约</th><th>方向</th><th>数量</th><th>开仓均价</th><th>标记价格</th><th>实际占用保证金</th><th>未实现盈亏</th><th>爆仓价格</th><th>杠杆</th><th>平仓</th></tr></thead><tbody>{positions.map((item) => <tr key={`${item.symbol}-${item.positionSide}`}><td><button className={styles.analysisButton} onClick={() => onAnalyze(item)} disabled={analysisLoadingSymbol === item.symbol}>{analysisLoadingSymbol === item.symbol ? "分析中" : "分析"}</button></td><td><button className={styles.symbolLink} onClick={() => onSelectSymbol(item.symbol)}><strong>{displayBinanceSymbol(item.symbol)}</strong><small>{quoteAssetForSymbol(item.symbol) ?? "USDT"} 永续</small></button></td><td className={item.side === "LONG" ? styles.up : styles.down}>{item.side === "LONG" ? "做多" : "做空"}</td><td>{item.quantity}</td><td>{formatPrice(item.entryPrice)}</td><td>{formatPrice(item.markPrice)}</td><td>{item.occupiedMargin === null ? "未知" : `${formatMoney(item.occupiedMargin)} USDT`}</td><td className={item.unrealizedPnl >= 0 ? styles.up : styles.down}>{item.unrealizedPnl >= 0 ? "+" : ""}{formatMoney(item.unrealizedPnl)}</td><td>{formatPrice(item.liquidationPrice)}</td><td>{item.leverage}x</td><td><ClosePositionAction symbol={item.symbol} quantity={item.quantity} canClose={canClose} disabledReason={closeDisabledReason} onClose={(percent) => onClose(item, percent)} /></td></tr>)}</tbody></table></div>;
}
