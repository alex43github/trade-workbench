...
type Props = { onChanged?: () => void; refreshToken?: number };
...
export default function LiveStrategyStatusList({ onChanged, refreshToken = 0 }: Props) {
    ...
    return <article key={strategy.id} className={`${styles.strategyStatusCard} ${strategy.status === "RECONCILIATION_REQUIRED" ? styles.liveStrategyRisk : ""}`}>
      <div className={styles.liveStrategyCardHeader}>
        <div className={styles.liveStrategyIdentity}>
          ...
          <span className={styles.liveStrategyRiskBadge} title={strategy.displayLeverageSource === "CURRENT_ACCOUNT" ? "旧策略未保存下单时杠杆，显示当前 Binance 合约杠杆" : strategy.displayLeverageSource === "UNAVAILABLE" ? "杠杆暂时无法读取" : "下单时记录的杠杆快照"}>{strategyRiskLabel(strategy.config.totalMarginUsdt, strategy.displayLeverage)}</span>
        </div>
      ...
      <ol>{strategy.orders.map((order) => <li key={order.id} className={order.status === "FILLED" ? styles.liveOrderFilled : ""}><span>{orderSourceLabel(order.clientOrderId)} · {orderStatusLabel[order.status]}</span><small>{formatOrder(order)}</small>{orderFailureIndicator(orderFailureReason(order))}{protectionIndicator(order.protection)}</li>)}</ol>
      {visibleHistoricalAttempts(strategy).length > 0 && <details><summary>历史订单尝试（只读）</summary><ol>{visibleHistoricalAttempts(strategy).map((attempt) => <li key={attempt.id}><span>第 {attempt.generation} 轮 · {orderSourceLabel(attempt.clientOrderId)} · {attemptTransition(attempt, strategy.currentGeneration?.generation ?? null)}</span><small>{attempt.side ?? "—"} · 价格 {attempt.price ?? "待定"}</small>{orderFailureIndicator(orderFailureReason(attempt))}</li>)}</ol></details>}
