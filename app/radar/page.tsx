...
import { isMultiTimeframeSnapshotFresh, type FineScreenSnapshot, type MultiTimeframeSnapshot } from "@/lib/radar/multitimeframe";
...
type AtrLifecycleDirection = "ALL" | "LONG" | "SHORT";
type AtrLifecycleRecord = {
  symbol: string;
  direction: "LONG" | "SHORT";
  status: "STRONG" | "WARNING" | "HISTORY";
  entryTime: number;
...
type AtrLifecycleDashboard = {
  status: "ready" | "degraded" | "pending";
  scannedAt: string | null;
  scanBucket: string | null;
  timezone: string;
  multiplier: number;
  lifecycles: AtrLifecycleRecord[];
  active: AtrLifecycleRecord[];
  strong: AtrLifecycleRecord[];
  warning: AtrLifecycleRecord[];
  history: AtrLifecycleRecord[];
...
type AtrLifecyclePayload = Omit<Partial<AtrLifecycleDashboard>, "warning"> & {
  warning?: AtrLifecycleRecord[] | string;
  warningMessage?: string;
  warningText?: string;
};

function createEmptyAtrLifecycleDashboard(): AtrLifecycleDashboard {
  return {
    status: "pending",
    scannedAt: null,
    scanBucket: null,
...
function normalizeAtrLifecycleDashboard(payload: AtrLifecyclePayload): AtrLifecycleDashboard {
  const empty = createEmptyAtrLifecycleDashboard();
  const { warning, warningMessage, warningText, ...rest } = payload;
  return {
    ...empty,
    ...rest,
    ...
    ...(typeof warning === "string" || warningMessage || warningText ? { warningMessage: typeof warning === "string" ? warning : warningMessage ?? warningText } : {}),
  };
}
...
const emptyReversalArchivePage: ReversalArchivePage = { items: [], page: 1, pageSize: 60, total: 0, totalPages: 1, sort: "signalTime", order: "desc" };

function normalizeReversalResponse(payload: Partial<ReversalResponse>): ReversalResponse {
  const legacyArchives = Array.isArray(payload.archives) ? payload.archives : null;
  return {
    status: payload.status ?? "pending",
    scans: payload.scans ?? {},
    dailyScans: payload.dailyScans,
    manualScans: payload.manualScans,
    archives: legacyArchives ? { ...emptyReversalArchivePage, items: legacyArchives, total: legacyArchives.length } : payload.archives ?? emptyReversalArchivePage,
...
type Filter = "composite" | "all" | "squeeze" | "candidate" | "concentrated" | "risk" | "ma30oi" | "atrband" | "reversal" | "vegas";
const fineConditionLabels: Record<FineCondition, string> = {
  LONG_15M_MA30: "多头 · 站上 15 分钟 MA30（K 线站上 15 分钟 MA30）",
  LONG_1H_MA30: "多头 · 站上 1 小时 MA30（K 线站上 1 小时 MA30）",
  LONG_4H_MA30: "多头 · 站上 4 小时 MA30（K 线站上 4 小时 MA30）",
  SHORT_15M_MA30: "空头 · 低于 15 分钟 MA30（K 线低于 15 分钟 MA30）",
  SHORT_1H_MA30: "空头 · 低于 1 小时 MA30（K 线低于 1 小时 MA30）",
  SHORT_4H_MA30: "空头 · 低于 4 小时 MA30（K 线低于 4 小时 MA30）",
};
const fineConditionIntervals: Record<FineCondition, "15m" | "1h" | "4h"> = {
  LONG_15M_MA30: "15m", LONG_1H_MA30: "1h", LONG_4H_MA30: "4h",
  SHORT_15M_MA30: "15m", SHORT_1H_MA30: "1h", SHORT_4H_MA30: "4h",
};
...
function formatOpenInterest(value: number | null | undefined) {
...
function formatLifecycleTime(value: number | null) {
  return typeof value === "number" && Number.isFinite(value) ? formatRadarTime(new Date(value)) : "—";
}

function formatAtrMultiple(value: number) {
  return Number.isFinite(value) ? `${value.toFixed(2)}×` : "—";
}

function reversalLevelCopy(value: ReversalArchive["reclaimLevel"]) {
  return value === "HIGH" ? "收回前实体高" : "收回前实体低";
}
...
function reversalIntervalLabel(interval: ReversalInterval) {
  return interval === "1w" ? "周线" : interval === "1d" ? "日线" : interval === "4h" ? "4H" : interval === "1h" ? "1H" : "15m";
}

function reversalElapsedBars(signalTime: number, interval: ReversalInterval, now = Date.now()) {
  const intervalMs: Record<ReversalInterval, number> = { "15m": 900_000, "1h": 3_600_000, "4h": 14_400_000, "1d": 86_400_000, "1w": 604_800_000 };
  return Math.max(0, Math.floor((now - signalTime) / intervalMs[interval]));
}

function formatReversalBodyRatio(value: number | undefined) {
  return typeof value === "number" && Number.isFinite(value) ? `${value.toFixed(2)}×` : "—";
}

function breakoutLookbackCopy(row: ReversalRow) {
  const bars = row.breakoutLookbackBars ?? 5;
  const suffix = row.breakoutLookbackCapped ? "+" : "";
...
function ReversalTable({ rows, title }: { rows: ReversalRow[]; title: string }) {
  type ActiveSort = "symbol" | "interval" | "bodyRatio" | "reclaimLevel" | "wickRatio" | "breakoutLookbackBars";
  const [sort, setSort] = useState<ActiveSort>("symbol");
  const [order, setOrder] = useState<ReversalArchiveOrder>("asc");
  const toggle = (next: ActiveSort) => {
    if (next === sort) setOrder((current) => current === "asc" ? "desc" : "asc");
    else { setSort(next); setOrder("asc"); }
  };
  const sortedRows = [...rows].sort((left, right) => {
    const leftValue = sort === "symbol" ? left.symbol : sort === "interval" ? left.interval : sort === "bodyRatio" ? left.bodyRatio ?? 0 : sort === "reclaimLevel" ? left.reclaimLevel : sort === "wickRatio" ? left.wickRatio : left.breakoutLookbackBars ?? 5;
    const rightValue = sort === "symbol" ? right.symbol : sort === "interval" ? right.interval : sort === "bodyRatio" ? right.bodyRatio ?? 0 : sort === "reclaimLevel" ? right.reclaimLevel : sort === "wickRatio" ? right.wickRatio : right.breakoutLookbackBars ?? 5;
...
function Ma30OiCandidateTable({ rows, direction, title }: { rows: Ma30OiCandidate[]; direction: "LONG" | "SHORT"; title: string }) {
  ...
  return <section className="reversal-direction"><div className="reversal-direction-title"><h4>{title}</h4><span>{rows.length} 个</span></div>{rows.length ? <table className="ma30-oi-table"><thead><tr><th>币种</th><th>当前 OI 数量</th><th>前日 OI</th><th>前 10 日均值</th><th>放大</th><th>{isLong ? "连续站上" : "连续低于"}</th></tr></thead><tbody>{rows.map((candidate) => <tr key={candidate.symbol}><td><a href={`/trade?symbol=${encodeURIComponent(candidate.symbol)}`}>{displayBinanceSymbol(candidate.symbol)}</a><small>收盘 {formatPrice(candidate.lastClose)} · MA30 {formatPrice(candidate.ma30)}</small></td><td>{formatOpenInterest(candidate.currentOi)}</td><td>{formatOpenInterest(candidate.previousDayOi)}</td><td>{formatOpenInterest(candidate.priorTenDayOiAverage)}</td><td className="positive">+{candidate.oiExpansionPct.toFixed(2)}%</td><td>{isLong ? candidate.consecutiveAboveMa : candidate.consecutiveBelowMa ?? 0} 根</td></tr>)}</tbody></table> : <div className="ma30-oi-empty">本轮没有符合条件的{isLong ? "多头" : "空头"}币种。</div>}</section>;
}

const atrLifecycleStatusCopy: Record<AtrLifecycleRecord["status"], string> = {
  STRONG: "强势",
  WARNING: "警示",
  HISTORY: "已结束",
};

function filterAtrLifecycles(rows: readonly AtrLifecycleRecord[], direction: AtrLifecycleDirection, query: string) {
...
function createPendingMultiTimeframe(symbols: string[], fineRequest?: FineScreenRequest): MultiTimeframeSnapshot {
  return { status: "pending", scannedAt: new Date().toISOString(), timezone: "Asia/Shanghai", symbols, bySymbol: {}, vegas: { "1h": [], "4h": [], "1d": [] }, vegasBearish: { "1h": [], "4h": [], "1d": [] }, scannedSymbols: 0, successfulSymbols: 0, failedSymbols: 0, progress: createScanProgress(symbols.length), warning: "正在读取 Binance Futures 最新已收盘 K 线", ...(fineRequest?.conditions.length ? { fine: { request: fineRequest, status: "pending", symbols, results: [], scannedSymbols: 0, matchedSymbols: 0, deepScannedSymbols: 0, failedSymbols: 0 } } : {}) };
}

function sameFineRequest(left: FineScreenRequest | undefined, right: FineScreenRequest) {
  if (!left) return right.conditions.length === 0;
  return left.mode === right.mode && left.conditions.length === right.conditions.length && right.conditions.every((condition) => left.conditions.includes(condition));
}

function conditionMatches(snapshot: MultiTimeframeSnapshot | null, symbol: string, condition: FineCondition) {
  const indicator = snapshot?.bySymbol[symbol]?.[fineConditionIntervals[condition]];
  if (!indicator || !Number.isFinite(indicator.close) || !Number.isFinite(indicator.ma30)) return false;
  return condition.startsWith("LONG_") ? indicator.close > indicator.ma30 : indicator.close < indicator.ma30;
}

function MultiTimeframeBucketBar({ symbols, scanSymbols, snapshot, request, onToggle, onMode, onScan, scanning }: { symbols: string[]; scanSymbols: string[]; snapshot: MultiTimeframeSnapshot | null; request: FineScreenRequest; onToggle: (condition: FineCondition) => void; onMode: (mode: FineCombinationMode) => void; onScan: () => void; scanning: boolean }) {
  const fresh = isMultiTimeframeSnapshotFresh(snapshot);
  const snapshotMatchesSymbols = fresh && sameSymbolSet(snapshot?.symbols ?? [], scanSymbols);
  const snapshotMatchesWindow = snapshotMatchesSymbols && sameFineRequest(snapshot?.fine?.request, request);
  ...
  const count = (condition: FineCondition) => uniqueSymbols(symbols).filter((symbol) => conditionMatches(snapshotMatchesSymbols ? snapshot : null, symbol, condition)).length;
  ...
  return <section className="ma30-bucket-panel" aria-label="已收盘 K 线 MA30 分档">
    <div className="ma30-bucket-heading"><div><strong>当前窗口后置精筛</strong><small>先用当前栏目候选粗筛，再按下方条件组合精筛并按评分排序</small></div><div className="ma30-bucket-actions"><span className={`ma30-bucket-status ${snapshot?.status ?? "pending"}`}>{status}</span><button type="button" onClick={onScan} disabled={scanning || !symbols.length}>{scanning ? "正在筛选…" : "立即筛选"}</button></div></div>
    <div className="fine-mode-toggle" role="group" aria-label="条件组合方式"><span>组合方式</span>{(["AND", "OR"] as const).map((mode) => <button type="button" key={mode} className={request.mode === mode ? "active" : ""} aria-pressed={request.mode === mode} onClick={() => onMode(mode)}>{mode}</button>)}<small>{request.mode === "AND" ? "同时满足全部已选条件" : "满足任一已选条件"}</small></div>
    <div className="ma30-bucket-direction"><strong>多头条件（多头：需站上）</strong><div className="ma30-bucket-buttons">
      {FINE_SCREEN_CONDITIONS.filter((condition) => condition.startsWith("LONG_")).map((condition) => <button type="button" key={condition} className={request.conditions.includes(condition) ? "active" : ""} aria-pressed={request.conditions.includes(condition)} onClick={() => onToggle(condition)}>{fineConditionLabels[condition]} <b>{count(condition)}</b></button>)}
    </div></div>
    <div className="ma30-bucket-direction"><strong>空头条件（空头：需低于）</strong><div className="ma30-bucket-buttons">
      {FINE_SCREEN_CONDITIONS.filter((condition) => condition.startsWith("SHORT_")).map((condition) => <button type="button" key={condition} className={request.conditions.includes(condition) ? "active" : ""} aria-pressed={request.conditions.includes(condition)} onClick={() => onToggle(condition)}>{fineConditionLabels[condition]} <b>{count(condition)}</b></button>)}
    </div></div>
    <small className="ma30-bucket-note">{!request.conditions.length ? "请选择一个或多个条件；不选条件时只保留粗筛结果" : !snapshotMatchesWindow ? "条件或当前窗口发生变化，请点击“立即筛选”" : !fresh && snapshot?.status === "ready" ? "快照已过期，请重新筛选" : snapshot?.warning ?? (symbols.length ? "正在读取 15m、1h、4h 已收盘 K 线" : "当前窗口暂无候选币种")}{skippedSymbols ? ` · ${skippedSymbols} 个名称不符合 Binance USDT 合约格式，未请求 K 线` : ""} · 缺失数据不会计入多头或空头结果</small>
  </section>;
}

function VegasBuckets({ snapshot, query }: { snapshot: MultiTimeframeSnapshot | null; query: string }) {
  return <div className="vegas-bucket-grid">
    {(["BULLISH", "BEARISH"] as const).flatMap((direction) => (["1h", "4h", "1d"] as const).map((interval) => {
      ...
      const matches = source.filter((symbol) => symbolMatchesQuery(symbol, query));
...
function FineScreenResults({ snapshot, request, symbols, query }: { snapshot: MultiTimeframeSnapshot | null; request: FineScreenRequest; symbols: string[]; query: string }) {
  const fine: FineScreenSnapshot | undefined = snapshot?.fine;
  const current = Boolean(fine && isMultiTimeframeSnapshotFresh(snapshot) && sameSymbolSet(snapshot?.symbols ?? [], symbols) && sameFineRequest(fine.request, request));
  const results = current ? (fine?.results ?? []).filter((candidate) => symbolMatchesQuery(candidate.symbol, query)) : [];
  return <section className="fine-screen-results" aria-label="多条件精筛结果">
    <div className="fine-screen-heading"><div><p className="section-kicker">TWO-STAGE FINE SCREEN</p><h3>精筛结果 · {request.mode}</h3><p>当前栏目先粗筛，再在候选集合内按已选条件精筛；所有结果按综合权重评分降序排列。</p></div><div className="fine-screen-status"><strong>{current ? results.length : 0} 个</strong><small>{request.conditions.length ? `已选 ${request.conditions.length} 个条件` : "尚未选择条件"}</small></div></div>
    {!request.conditions.length ? <div className="fine-screen-empty">请选择一个或多个条件，然后点击“立即筛选”。</div> : !current ? <div className="fine-screen-empty">当前条件或栏目候选已变化，请点击“立即筛选”生成最新精筛结果。</div> : results.length ? <div className="fine-screen-grid">{results.map((candidate, index) => <a className="fine-screen-card" key={candidate.symbol} href={`/trade?symbol=${encodeURIComponent(candidate.symbol)}`}><div className="fine-screen-card-top"><span className="fine-screen-rank">{String(index + 1).padStart(2, "0")}</span><strong>{displayBinanceSymbol(candidate.symbol)}</strong><b>{candidate.score.toFixed(0)} 分</b></div><div className="fine-screen-card-meta"><span className={candidate.direction === "SHORT" ? "negative" : candidate.direction === "LONG" ? "positive" : ""}>{candidate.direction === "MIXED" ? "多空混合" : candidate.direction === "LONG" ? "多头" : candidate.direction === "SHORT" ? "空头" : "中性"}</span><span>数据完整度 {candidate.dataCompleteness}%</span></div><div className="fine-screen-condition-list">{candidate.matchedConditions.map((condition) => <span key={condition}>{fineConditionLabels[condition]}</span>)}</div><div className="fine-screen-score-breakdown"><span>趋势 {candidate.componentScores.trend.toFixed(0)}</span><span>Vegas {candidate.componentScores.vegas.toFixed(0)}</span><span>量能 {candidate.componentScores.volume.toFixed(0)}</span><span>OI {candidate.componentScores.oi.toFixed(0)}</span><span>波动 {candidate.componentScores.volatility.toFixed(0)}</span></div></a>)}</div> : <div className="fine-screen-empty">本轮没有同时满足当前 {request.mode} 组合的币种。</div>}
  </section>;
}
...
export default function Home() {
  ...
  const [atrBand, setAtrBand] = useState<AtrLifecycleDashboard | null>(null);
  const [atrBandScanning, setAtrBandScanning] = useState(false);
  const [atrLifecycleDirection, setAtrLifecycleDirection] = useState<AtrLifecycleDirection>("ALL");
  const [reversal, setReversal] = useState<ReversalResponse | null>(null);
  const [reversalScanning, setReversalScanning] = useState(false);
  const [reversalInterval, setReversalInterval] = useState<ReversalInterval>("4h");
  const [archiveQuery, setArchiveQuery] = useState<Pick<ReversalArchivePage, "page" | "sort" | "order">>({ page: 1, sort: "signalTime", order: "desc" });
  ...
  const [selectedFineConditions, setSelectedFineConditions] = useState<FineCondition[]>([]);
  const [fineMode, setFineMode] = useState<FineCombinationMode>("AND");
 ...
 const { fontScale } = useFontScale();
  const multiTimeframeSymbols = useMemo(() => {
    ...
    const compositeSymbols = composite?.candidates?.map((candidate) => candidate.symbol) ?? [];
    const symbols = filter === "composite"
      ? (compositeSymbols.length ? compositeSymbols : (data?.coins ?? []).map((coin) => coin.symbol))
      : filter === "reversal"
      ? reversalSymbols
      : filter === "ma30oi"
  ...
  }, [composite, data, filter, ma30Oi, reversal]);
  const fineRequest = useMemo<FineScreenRequest>(() => ({ conditions: selectedFineConditions, mode: fineMode }), [selectedFineConditions, fineMode]);
  ...
  async function runMultiTimeframeNow(symbols = multiTimeframeSymbols, request = fineRequest) {
    if (multiTimeframeScanning || !symbols.length) return;
    setMultiTimeframeScanning(true);
    setMultiTimeframe(createPendingMultiTimeframe(symbols, request));
    try {
      const response = await fetch("/api/radar/multitimeframe", { method: "POST", headers: { "content-type": "application/json", "x-radar-manual": "1" }, body: JSON.stringify({ symbols, conditions: request.conditions, mode: request.mode }) });
      const payload = await response.json().catch(() => ({})) as Partial<MultiTimeframeSnapshot>;
      if (!response.ok) {
        const transient = isTransientScanTransportFailure(response.status);
        const warning = response.status === 401 ? operatorLoginWarning : transient ? transientScanWarning : payload.warning || "多周期筛选暂时不可用";
        setMultiTimeframe({ ...createPendingMultiTimeframe(symbols, request), status: transient ? "pending" : "degraded", warning });
      ...
      setMultiTimeframe({ ...createPendingMultiTimeframe(symbols, request), status: "pending", warning: transientScanWarning });
    } finally {
  ...
  function scanCurrentWindow() {
    return runMultiTimeframeNow(multiTimeframeSymbols, fineRequest);
  }
  ...
  async function loadAtrBand() {
    try {
      const response = await fetch("/api/radar/atr-band", { cache: "no-store" });
      const payload = await response.json().catch(() => ({})) as AtrLifecyclePayload;
      if (!response.ok) throw new Error(typeof payload.warning === "string" ? payload.warning : "MA30 ± ATR 生命周期快照暂时不可用");
      setAtrBand(normalizeAtrLifecycleDashboard(payload));
    } catch (reason) {
      setAtrBand((current) => ({
        ...(current ?? createEmptyAtrLifecycleDashboard()),
        warningMessage: reason instanceof Error ? reason.message : transientScanWarning,
      }));
    }
  }

  async function runAtrBandNow() {
    if (atrBandScanning) return;
    setAtrBandScanning(true);
    try {
      const response = await fetch("/api/radar/atr-band", { method: "POST", headers: { "x-radar-manual": "1" } });
      const payload = await response.json().catch(() => ({})) as AtrLifecyclePayload;
      if (!response.ok) {
        const message = response.status === 401
          ? operatorLoginWarning
          : typeof payload.warning === "string" ? payload.warning : "MA30 ± ATR 生命周期扫描暂时不可用";
        if (response.status === 401) router.push("/signin?return_to=/radar");
        setAtrBand((current) => ({ ...(current ?? createEmptyAtrLifecycleDashboard()), status: "degraded", warningMessage: message }));
        return;
      }
      setAtrBand(normalizeAtrLifecycleDashboard(payload));
      if (payload.status === "pending") {
        for (let attempt = 0; attempt < 600; attempt += 1) {
          await new Promise((resolve) => setTimeout(resolve, 1_000));
          const latestResponse = await fetch("/api/radar/atr-band", { cache: "no-store" });
          if (!latestResponse.ok) break;
          const latest = await latestResponse.json().catch(() => ({})) as AtrLifecyclePayload;
          setAtrBand(normalizeAtrLifecycleDashboard(latest));
          if (latest.status !== "pending") break;
        }
      }
    } catch {
      setAtrBand((current) => ({ ...(current ?? createEmptyAtrLifecycleDashboard()), status: "pending", warningMessage: transientScanWarning }));
    } finally {
      setAtrBandScanning(false);
    }
  }
  ...
  async function loadReversal(nextQuery = archiveQuery) {
    try {
      const search = new URLSearchParams({ page: String(nextQuery.page), sort: nextQuery.sort, order: nextQuery.order });
      const response = await fetch(`/api/radar/reversal?${search}`, { cache: "no-store" });
      ...
      if (!response.ok) {
        ...
        setReversal({ status: transient ? "pending" : "degraded", scans: {}, archives: emptyReversalArchivePage, warning: message, diagnostic: transient ? undefined : createRadarDiagnostic(response.status, message) });
        return;
      }
      setReversal(normalizeReversalResponse(payload));
    } catch {
      setReversal({ status: "pending", scans: {}, archives: emptyReversalArchivePage, warning: transientScanWarning, diagnostic: undefined });
    }
  }
  ...
  async function pollReversalScan() {
    for (let attempt = 0; attempt < 600; attempt += 1) {
      ...
      try {
        ...
        setReversal((current) => ({ ...(current ?? { scans: {}, archives: emptyReversalArchivePage }), status: "pending", warning: transientScanWarning, diagnostic: undefined }));
        return;
      }
      if (!response.ok) {
        const transient = isTransientScanTransportFailure(response.status) && !payload.diagnostic;
        const message = transient ? transientScanWarning : payload.warning || "扫描状态暂时不可用";
        setReversal((current) => ({ ...(current ?? { scans: {}, archives: emptyReversalArchivePage }), status: transient ? "pending" : "degraded", warning: message, diagnostic: transient ? undefined : createRadarDiagnostic(response.status, message) }));
        return;
      }
      setReversal(normalizeReversalResponse(payload));
      if (payload.status !== "pending") return;
    ...
    const message = "扫描超过 10 分钟仍未完成，请稍后查看快照";
    setReversal((current) => ({ ...(current ?? { scans: {}, archives: emptyReversalArchivePage }), status: "pending", warning: message, diagnostic: createRadarDiagnostic(408, message) }));
  }

  async function runReversalNow() {
    if (reversalScanning) return;
    setReversalScanning(true);
    setReversal((current) => ({ ...(current ?? { scans: {}, archives: emptyReversalArchivePage }), status: "pending", warning: `正在筛选 ${reversalIntervalLabel(reversalInterval)} 的结构超强势，请耐心等待`, diagnostic: undefined }));
    try {
      const response = await fetch(`/api/radar/reversal?interval=${reversalInterval}`, { method: "POST", headers: { "x-radar-manual": "1" } });
      ...
      if (!response.ok) {
        ...
        setReversal((current) => ({ ...(current ?? { scans: {}, archives: emptyReversalArchivePage }), status: "pending", warning: message, diagnostic: transient ? undefined : createRadarDiagnostic(response.status, message) }));
        if (response.status === 401) router.push("/signin?return_to=/radar");
        return;
      }
      setReversal(normalizeReversalResponse(payload));
  ...
  useEffect(() => {
    ...
    const atrBandTimer = window.setTimeout(() => void loadAtrBand(), 0);
    ...
    return () => {
      ...
      window.clearTimeout(atrBandTimer);
 ...
 useEffect(() => {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      void loadMultiTimeframe().then((snapshot) => {
        if (!sameSymbolSet(snapshot?.symbols ?? [], multiTimeframeSymbols) || !isMultiTimeframeSnapshotFresh(snapshot)) {
  ...
  const atrLifecycleGroups = useMemo(() => ({
    strong: filterAtrLifecycles(atrBand?.strong ?? [], atrLifecycleDirection, normalizedQuery),
    warning: filterAtrLifecycles(atrBand?.warning ?? [], atrLifecycleDirection, normalizedQuery),
    history: filterAtrLifecycles(atrBand?.history ?? [], atrLifecycleDirection, normalizedQuery),
  }), [atrBand, atrLifecycleDirection, normalizedQuery]);
  ...
  const filteredCoins = useMemo(() => baseRadarCoins.filter((coin) => {
    ...
    return matchesQuery;
  }), [baseRadarCoins, normalizedQuery]);
  ...
  const activeReversalScans = reversal?.manualScans?.[reversalInterval] ? { [reversalInterval]: reversal.manualScans[reversalInterval] } : reversal?.scans ?? {};
 const reversalCandidates = Object.values(activeReversalScans).flatMap((scan) => scan?.candidates ?? []);
  const baseWindowSymbols = filter === "composite"
    ? uniqueSymbols((composite?.candidates ?? []).map((candidate) => candidate.symbol).length ? (composite?.candidates ?? []).map((candidate) => candidate.symbol) : baseRadarCoins.map((coin) => coin.symbol))
    : filter === "ma30oi"
  ...
  const bucketedReversalCandidates = reversalCandidates.filter((candidate) => {
    return symbolMatchesQuery(candidate.symbol, normalizedQuery);
  });
  const bucketedMa30OiCandidates = (ma30Oi?.candidates ?? []).filter((candidate) => {
    return symbolMatchesQuery(candidate.symbol, normalizedQuery);
  });
  const longMa30OiCandidates = bucketedMa30OiCandidates.filter((candidate) => (candidate.direction ?? "LONG") === "LONG");
  const shortMa30OiCandidates = bucketedMa30OiCandidates.filter((candidate) => candidate.direction === "SHORT");
  const reversalProgresses = (["15m", "1h", "4h", "1d", "1w"] as const).flatMap((interval) => {
    const progress = reversal?.manualScans?.[interval]?.progress ?? reversal?.scans[interval]?.progress;
    return progress ? [{ label: reversalIntervalLabel(interval), progress }] : [];
  });
  const archive = reversal?.archives ?? emptyReversalArchivePage;
  const changeArchiveSort = (sort: ReversalArchiveSort) => {
    const next = { page: 1, sort, order: archive.sort === sort && archive.order === "desc" ? "asc" as const : "desc" as const };
    setArchiveQuery(next);
    void loadReversal(next);
  };
  const changeArchivePage = (page: number) => {
    const next = { page, sort: archive.sort, order: archive.order };
    setArchiveQuery(next);
    void loadReversal(next);
  };
  ...
  return (
    <main className="app-shell radar-terminal" data-theme={resolvedTheme} style={{ "--site-font-scale": fontScale } as React.CSSProperties}>
      <aside className="radar-sidebar">
        ...
        <div className="radar-sidebar-foot"><i className={data?.mode === "live" ? "connected" : ""} /><div><strong>{data?.mode === "live" ? "数据源实时" : data?.mode === "hybrid" ? "Binance 实时 · 辅助源部分" : "当前演示模式"}</strong><small>{data ? relativeTime(data.updatedAt) : "连接中"}</small></div></div>
      </aside>
      <div className="radar-app-main">
      <header className="radar-top-header">
        <div><small>HOME / MARKET INTELLIGENCE</small><h1>妖币雷达</h1></div>
        <div className="radar-header-controls"><div className="radar-theme-switch" aria-label="主题选择">{(["dark", "light", "system"] as ThemeMode[]).map((item) => <button key={item} className={themeMode === item ? "selected" : ""} onClick={() => setThemeMode(item)}>{item === "dark" ? "深色" : item === "light" ? "浅色" : "跟随系统"}</button>)}</div><FontControl /><span className="radar-live-status"><i className={data?.mode === "live" ? "connected" : ""} />{data?.mode === "live" ? "全源实时" : data?.mode === "hybrid" ? "Binance 实时 · 辅助源部分" : "演示行情"}</span><button className="refresh-button" onClick={() => void Promise.all([loadRadar(), loadMa30Oi(), loadAtrBand(), loadReversal(), loadComposite()])} disabled={loading}>{loading ? "正在刷新" : "刷新"}</button></div>
      </header>
        ...
        <div className="radar-panel">
          ...
          <div className="toolbar">
            <div className="filter-tabs" role="tablist" aria-label="筛选热门币">
              {([
                ["composite", "综合榜"], ["all", "全部雷达"], ["squeeze", "逼空重点"], ["candidate", "候选"], ["concentrated", "筹码集中"], ["risk", "风险检查"], ["ma30oi", "MA30 × OI 增仓"], ["atrband", "MA30 ± ATR 持续"], ["reversal", "结构超强势"], ["vegas", "Vegas 强势"],
          ...
          <MultiTimeframeBucketBar symbols={baseWindowSymbols} scanSymbols={multiTimeframeSymbols} snapshot={multiTimeframe} request={fineRequest} onToggle={(condition) => setSelectedFineConditions((current) => current.includes(condition) ? current.filter((item) => item !== condition) : [...current, condition])} onMode={setFineMode} onScan={() => void scanCurrentWindow()} scanning={multiTimeframeScanning} />
          ...
          <FineScreenResults snapshot={multiTimeframe} request={fineRequest} symbols={multiTimeframeSymbols} query={normalizedQuery} />
         ...
         {filter === "composite" ? <CompositePanel snapshot={composite} query={normalizedQuery} /> : filter === "atrband" ? (
            <div className="atr-lifecycle-panel">
              ...
              <div className="atr-lifecycle-direction-toggle" role="tablist" aria-label="ATR 生命周期方向">
                {(["ALL", "LONG", "SHORT"] as const).map((direction) => {
                  const label = direction === "ALL" ? "全部" : direction === "LONG" ? "多头" : "空头";
                  const count = (atrBand?.strong ?? []).concat(atrBand?.warning ?? [], atrBand?.history ?? []).filter((lifecycle) => direction === "ALL" || lifecycle.direction === direction).length;
                  return <button key={direction} type="button" role="tab" aria-selected={atrLifecycleDirection === direction} aria-pressed={atrLifecycleDirection === direction} className={atrLifecycleDirection === direction ? "active" : ""} onClick={() => setAtrLifecycleDirection(direction)}>{label}<small>{count}</small></button>;
                })}
              </div>
            ...
            <div className="reversal-panel">
              <div className="reversal-heading"><div><p className="section-kicker">STRUCTURAL REVERSAL RADAR</p><h3>结构超强势</h3><p>仅标记：突破前 5 根结构极值、实体不少于前 5 根平均实体，并完全收回前一根实体的已收盘 K 线。</p></div><div className="reversal-actions"><span className={`reversal-status ${reversal?.diagnostic ? "degraded" : reversal?.status ?? "pending"}`}>{reversalScanning ? "筛选中" : reversal?.diagnostic ? "连接失败" : reversal?.status === "ready" ? "已归档" : reversal?.status === "degraded" ? "数据不足" : "待执行"}</span><button type="button" onClick={() => void runReversalNow()} disabled={reversalScanning}>{reversalScanning ? "正在筛选…" : `筛选 ${reversalIntervalLabel(reversalInterval)}`}</button></div></div>
              <div className="filter-tabs reversal-interval-tabs" role="tablist" aria-label="结构超强势周期">{(["15m", "1h", "4h", "1d", "1w"] as const).map((interval) => <button key={interval} type="button" role="tab" aria-selected={reversalInterval === interval} className={reversalInterval === interval ? "active" : ""} onClick={() => setReversalInterval(interval)}>{reversalIntervalLabel(interval)} <small>{reversal?.dailyScans?.[interval]?.candidates.length ?? reversal?.scans?.[interval]?.candidates.length ?? 0}</small></button>)}</div>
              <ManualProgress active={reversalScanning} progresses={reversalProgresses} label={`正在读取 ${reversalIntervalLabel(reversalInterval)} 已收盘 K 线`} estimate="数十秒至数分钟，取决于币种数量和网络" />
              <div className="reversal-meta"><span>每日快照：北京时间 08:05</span><span>当前：{reversal?.manualScans?.[reversalInterval] ? `手动筛选 ${formatRadarTime(reversal.manualScans[reversalInterval]!.scannedAt)}` : reversal?.dailyScans?.[reversalInterval] ? `每日归档 ${formatRadarTime(reversal.dailyScans[reversalInterval]!.scannedAt)}` : "尚未生成"}</span>{reversalProgresses.length > 0 && <span>USDT 永续：{reversalProgresses.map(({ label, progress }) => `${label} ${progress.scannedSymbols}/${progress.totalSymbols}`).join(" · ")} · 命中：{reversalProgresses.reduce((sum, item) => sum + item.progress.matchedSymbols, 0)}</span>}<span>归档：{archive.total} 条</span></div>
           ...
           <div className="ma30-oi-panel">
              ...
              {bucketedMa30OiCandidates.length ? <div className="reversal-direction-grid"><Ma30OiCandidateTable title="多头 · 连续站上 MA30 + OI 扩张" rows={longMa30OiCandidates} direction="LONG" /><Ma30OiCandidateTable title="空头 · 连续低于 MA30 + OI 扩张" rows={shortMa30OiCandidates} direction="SHORT" /></div> : <div className="ma30-oi-empty">暂无符合条件的币种。{ma30Oi?.warning ?? "每日 08:00 扫描后显示结果。"}</div>}
            </div>
          ) : error ? (
