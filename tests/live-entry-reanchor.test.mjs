...
function createHarness(options = {}) {
  ...
  const dependencies = {
    ...
    findOrder: async ({ clientOrderId }) => {
      ...
      return attempt ? {
        orderId: attempt.exchangeOrderId, clientOrderId, status: attempt.status === "FILLED" ? "FILLED" : "NEW", executedQty: attempt.executedQuantity, avgPrice: attempt.averageFillPrice ?? "0",
...
test("settles a partial fill and creates orders only for the target remainder", async () => {
  const { dependencies, events, strategy } = createHarness({ attempts: [submittedAttempt(1, "0.4"), ...Array.from({ length: 4 }, (_, index) => submittedAttempt(index + 2))] });
