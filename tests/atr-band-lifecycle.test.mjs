...
test("ATR lifecycle scan exposes the persisted full-market hourly MA30 ± 1 ATR scanner", async () => {
  ...
  assert.match(route, /multiplier: 1/);
  assert.match(maintenance, /MA30 ± 1 ATR 机器自选由独立低频任务扫描/);
  assert.match(maintenance, /x-workbench-skip-atr-band/);
...
test("flat ATR data keeps lifecycle distance metrics finite", async () => {
  ...
  assert.equal(lifecycle?.status, "STRONG");
  assert.ok(Number.isFinite(lifecycle?.maxAtrMultiple));
