...
test("strategy APIs require an operator outside local development", async () => {
  process.env.STREETLIGHT_LOCAL_TEST_MODE = "false";
  const { GET } = await import("../app/api/trade/strategies/route.ts");
  const response = await GET(new Request("http://localhost/api/trade/strategies"));
  assert.equal(response.status, 401);
  delete process.env.STREETLIGHT_LOCAL_TEST_MODE;
});

test("creates a PAPER strategy with immutable website order IDs and lists it", async () => {
  ...
  assert.equal(result.body.strategy.config.mode, "PAPER");
  assert.equal(result.body.strategy.events.at(-1).type, "CREATED");

  const { GET } = await import("../app/api/trade/strategies/route.ts");
  const response = await GET(new Request("http://localhost/api/trade/strategies?limit=10"));
  const body = await response.json();
  assert.equal(response.status, 200);
  assert.equal(body.strategies.some((strategy) => strategy.id === result.body.strategy.id), true);
});

test("strategy source controls the human-readable order ID prefix", async () => {
  const telegram = await postStrategy({ ...validDraft, origin: "TELEGRAM", idempotencyKey: "source-telegram-1" });
  const web = await postStrategy({ ...validDraft, origin: "WEB", idempotencyKey: "source-web-1" });
  assert.equal(telegram.status, 201);
  assert.equal(web.status, 201);
  assert.equal(telegram.body.strategy.origin, "TELEGRAM");
  assert.equal(web.body.strategy.origin, "WEB");
  assert.ok(telegram.body.strategy.legs.every((leg) => /^tele\d{4}$/.test(leg.websiteOrderId)));
  assert.ok(web.body.strategy.legs.every((leg) => /^web\d{4}$/.test(leg.websiteOrderId)));
});

test("rejects unknown fields and duplicate idempotency keys", async () => {
  const unknown = await postStrategy({ ...validDraft, apiKey: "must-not-persist" });
  assert.equal(unknown.status, 400);

  const first = await postStrategy({ ...validDraft, idempotencyKey: "strategy-draft-1" });
  const second = await postStrategy({ ...validDraft, idempotencyKey: "strategy-draft-1" });
  assert.equal(first.status, 201);
  assert.equal(second.status, 409);
});

test("persists independent entry lots, refreshes, and cancellation audit without deletion", async () => {
  const created = await postStrategy({ ...validDraft, idempotencyKey: "strategy-ledger-1" });
  assert.equal(created.status, 201);
  const { cancelStrategy, getStrategy, recordEntryFill, recordLotExit, recordRefresh } = await import("../lib/trade/strategies.ts");
  const strategy = created.body.strategy;
  const firstLeg = strategy.legs[0];

  const firstLot = await recordEntryFill(strategy.id, { legId: firstLeg.id, quantity: 0.25, price: 100, sourceFillId: "paper-fill-1" });
  const secondLot = await recordEntryFill(strategy.id, { legId: firstLeg.id, quantity: 0.1, price: 101, sourceFillId: "paper-fill-2" });
  assert.notEqual(firstLot.id, secondLot.id);
  await recordLotExit(strategy.id, { lotId: firstLot.id, quantity: 0.1, realizedGrossPnl: 5, sourceExitId: "paper-exit-1" });
  await recordRefresh(strategy.id, { legId: strategy.legs[1].id, closedCandleId: "1h:42", price: 99, requestedQuantity: 0.3 });
  const canceled = await cancelStrategy(strategy.id, "USER_REQUEST");
  assert.equal(canceled?.status, "CANCELED");

  const persisted = await getStrategy(strategy.id);
  assert.equal(persisted?.lots.length, 2);
  assert.equal(persisted?.events.some((event) => event.type === "CANCELED"), true);
  assert.equal(persisted?.events.some((event) => event.type === "REFRESHED"), true);
});
