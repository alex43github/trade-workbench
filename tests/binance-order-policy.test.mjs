...
test("allows a Hedge Mode market close with an explicit position side and no reduceOnly", async () => {
  const { validateOrderPayload } = await import("../binance-gateway/order-policy.mjs");
  const market = new URLSearchParams({ symbol: "ETHUSDC", side: "SELL", type: "MARKET", quantity: "1.346", positionSide: "LONG", newClientOrderId: "alexMC1234567890123456789012345678" }).toString();
  assert.equal(validateOrderPayload("POST", "/fapi/v1/order", market), null);
});

test("allows Hedge Mode protective orders with positionSide and without reduceOnly", async () => {
  const { validateOrderPayload } = await import("../binance-gateway/order-policy.mjs");
  const stop = new URLSearchParams({ symbol: "ETHUSDC", side: "SELL", type: "STOP_MARKET", stopPrice: "2400", quantity: "1.346", positionSide: "LONG", newClientOrderId: "webSL1234567890123456789012345678" }).toString();
  assert.equal(validateOrderPayload("POST", "/fapi/v1/order", stop), null);
});
...
test("allows only server-marked market entries for Hedge Mode", async () => {
  ...
  const entry = new URLSearchParams({
    ...
    newClientOrderId: "webMKabcdef0123456789abcdef012345",
  }).toString();
  ...
  assert.match(validateOrderPayload("POST", "/fapi/v1/order", reduceOnly), /订单参数/);
});

test("requires an explicit position side on every Post Only entry order", async () => {
  const { validateOrderPayload } = await import("../binance-gateway/order-policy.mjs");
  const base = { symbol: "BTCUSDT", side: "BUY", type: "LIMIT", timeInForce: "GTX", price: "100", quantity: "0.01", newClientOrderId: "webIN10000001" };
