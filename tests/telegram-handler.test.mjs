...
test("quick order collects the requested fields and shows the default summary", async () => {
  ...
  assert.match(reply.text, /选择交易所/);
  await handleAuthorizedTelegramUpdate(update(6, "CALLBACK", "tg:act:exchange_binance_01", "901"), dependencies);
  reply = await handleAuthorizedTelegramUpdate(update(7, "MESSAGE", "BTCUSDT", "901"), dependencies);
  assert.ok(reply.replyMarkup.inline_keyboard.flat().some((button) => button.text === "做多"));
  reply = await handleAuthorizedTelegramUpdate(update(8, "CALLBACK", "tg:act:quick_side_long_01", "901"), dependencies);
  assert.match(reply.replyMarkup.inline_keyboard.flat().map((button) => button.text).join(" "), /15分钟.*1小时.*4小时.*1天/);
  reply = await handleAuthorizedTelegramUpdate(update(9, "CALLBACK", "tg:act:quick_tf_1h_01", "901"), dependencies);
  assert.ok(reply.replyMarkup.inline_keyboard.flat().some((button) => button.text === "50 USDT"));
  reply = await handleAuthorizedTelegramUpdate(update(10, "CALLBACK", "tg:act:qk_b2_01", "901"), dependencies);
...
test("Telegram shows the selected symbol current leverage before asking for direction", async () => {
  ...
  await handleAuthorizedTelegramUpdate(update(37, "CALLBACK", "tg:act:exchange_binance_01", "912"), dependencies);
  const quickSymbol = await handleAuthorizedTelegramUpdate(update(38, "MESSAGE", "HEMIUSDT", "912"), dependencies);
...
test("quick order accepts a custom total margin", async () => {
  ...
  await handleAuthorizedTelegramUpdate(update(23, "CALLBACK", "tg:act:exchange_binance_01", "910"), dependencies);
  await handleAuthorizedTelegramUpdate(update(24, "MESSAGE", "ETHUSDT", "910"), dependencies);
...
test("Telegram默认下单提供25 USDT并拆分为5笔每笔5 USDT", async () => {
  ...
  await handleAuthorizedTelegramUpdate(update(42, "CALLBACK", "tg:act:exchange_binance_01", "913"), dependencies);
  await handleAuthorizedTelegramUpdate(update(43, "MESSAGE", "BTCUSDT", "913"), dependencies);
...
test("Telegram完整策略将5笔作为默认分笔数量", async () => {
  ...
  await handleAuthorizedTelegramUpdate(update(48, "CALLBACK", "tg:act:exchange_binance_01", "914"), dependencies);
  await handleAuthorizedTelegramUpdate(update(49, "MESSAGE", "BTCUSDT", "914"), dependencies);
...
test("quick order confirmation submits five equal-margin legs with defaults", async () => {
  ...
  await handleAuthorizedTelegramUpdate(update(30, "CALLBACK", "tg:act:exchange_binance_01", "911"), dependencies);
  await handleAuthorizedTelegramUpdate(update(31, "MESSAGE", "BTCUSDT", "911"), dependencies);
...
test("Telegram live wizard submits a multi-leg strategy with tele client ids", async () => {
  ...
  await handleAuthorizedTelegramUpdate(update(11, "CALLBACK", "tg:act:exchange_binance_01", "902"), dependencies);
  const symbolReply = await handleAuthorizedTelegramUpdate(update(12, "MESSAGE", "BTCUSDT", "902"), dependencies);
...
test("Telegram protection flow binds a fixed level to the selected manual source", async () => {
  ...
  await handleAuthorizedTelegramUpdate(update(30, "CALLBACK", "tg:act:home_protection_01", "903"), dependencies);
  let reply = await handleAuthorizedTelegramUpdate(update(31, "CALLBACK", "tg:act:exchange_binance_01", "903"), dependencies);
  ...
  const percent = reply.replyMarkup.inline_keyboard.flat().find((button) => button.text.includes("100%"));
  assert.ok(percent);
  reply = await handleAuthorizedTelegramUpdate(update(32, "CALLBACK", percent.callback_data, "903"), dependencies);
  const sl = reply.replyMarkup.inline_keyboard.flat().find((button) => button.text.includes("止损策略"));
  assert.ok(sl);
  await handleAuthorizedTelegramUpdate(update(33, "CALLBACK", sl.callback_data, "903"), dependencies);
  const level = (await handleAuthorizedTelegramUpdate(update(34, "CALLBACK", "tg:act:alex_sl_level_01", "903"), dependencies));
  assert.match(level.text, /支撑阻力/);
  const confirmation = await handleAuthorizedTelegramUpdate(update(35, "MESSAGE", "2100", "903"), dependencies);
  const confirm = confirmation.replyMarkup.inline_keyboard.flat().find((button) => button.text.includes("确认挂策略单"));
  assert.ok(confirm);
  const result = await handleAuthorizedTelegramUpdate(update(36, "CALLBACK", confirm.callback_data, "903"), dependencies);
