...
test("orchestrator isolates R1, anonymizes R2 and keeps expert identity in R3", async () => {
  ...
  const runner = async (input) => {
    ...
    return {
      ...
      triggerConditions: input.expert.id === "jingxin" ? [] : ["close confirms"], machineTrigger: input.expert.id === "jingxin" ? null : { type: "PRICE_IN_ZONE", timeframe: "1h", price: null }, entryZone: input.expert.id === "jingxin" ? null : { low: 100, high: 101 },
...
test("orchestrator retries one failed expert without losing a valid three-expert council", async () => {
  ...
  const runner = async (input) => {
    ...
    return {
      ...
      triggerConditions: ["close confirms"], machineTrigger: { type: "PRICE_IN_ZONE", timeframe: "1h", price: null }, entryZone: { low: 100, high: 101 }, invalidation: "structure breaks", stopPrice: 98, targets: [104], managementPlan: "manage",
