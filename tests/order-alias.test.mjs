...
test("manual Binance order aliases are stable and use ios numbering", async () => {
  ...
  assert.equal(first, "ios0001");
  assert.equal(replay, first);
  assert.equal(second, "ios0002");
  assert.equal(unicode, "ios0003");
});
