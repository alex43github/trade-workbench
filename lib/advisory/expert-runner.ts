...
export async function runExpertRound(input: ExpertRunnerInput): Promise<DecisionContract> {
  ...
  const response = await fetch("https://api.openai.com/v1/responses", { method: "POST", headers: { authorization: `Bearer ${apiKey}`, "content-type": "application/json" }, signal: AbortSignal.timeout(45_000), body: JSON.stringify({
    model: process.env.OPENAI_MODEL || "gpt-5", reasoning: { effort: "medium" }, store: false,
    input: [{ role: "developer", content: `你是${input.expert.name}体系的独立研究专家。\n${EXPERT_GUIDES[input.expert.id]}\n只输出条件式研究建议，不承诺收益，不发送真实订单。严格依据输入中的已收盘日线、4H、1H数据。R1不得推测其他专家意见；R2只评价匿名论点；账户决定与市场判断分开。sourceRefs 只能引用上述指南已提供的真实来源，不得编造引用。` }, { role: "user", content: JSON.stringify(input) }],
