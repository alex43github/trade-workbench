...
import { invokeStructuredModel, invokeStructuredModelWithFallback, type CompatibleTarget } from "./model-gateway.ts";
...
const schema = {
  ...
  properties: {
    consultationId:{type:"string"},expertId:{type:"string",enum:["ict","street","jingxin","bitlanglang"]},round:{type:"string",enum:["R1","R2","R3"]},skillVersion:{type:"string"},snapshotHash:{type:"string"},symbol:{type:"string"},marketRegime:{type:"string"},direction:{type:"string",enum:["LONG","SHORT","NEUTRAL"]},setupName:{type:"string"},contextTimeframe:{type:"string"},executionTimeframe:{type:"string"},validUntil:{type:"string"},triggerConditions:{type:"array",items:{type:"string"}},machineTrigger:{anyOf:[{type:"null"},{type:"object",additionalProperties:false,properties:{type:{type:"string",enum:["PRICE_IN_ZONE","CLOSE_BREAKOUT","CLOSE_BREAKDOWN"]},timeframe:{type:"string",enum:["1h","4h"]},price:{anyOf:[{type:"null"},{type:"number"}]}},required:["type","timeframe","price"]}]},entryZone:{anyOf:[{type:"null"},{type:"object",additionalProperties:false,properties:{low:{type:"number"},high:{type:"number"}},required:["low","high"]}]},invalidation:{type:"string"},stopPrice:{anyOf:[{type:"null"},{type:"number",exclusiveMinimum:0}]},targets:{type:"array",items:{type:"number"}},managementPlan:{type:"string"},leverage:{type:"integer",minimum:1,maximum:10},marginUsdt:{type:"number",minimum:0},maxLossUsdt:{type:"number",minimum:0},expectedRr:{type:"number",minimum:0},triggerProbability:{type:"number",minimum:0,maximum:100},winProbabilityGivenTrigger:{type:"number",minimum:0,maximum:100},evidenceCompleteness:{type:"number",minimum:0,maximum:100},supportingEvidence:{type:"array",items:{type:"string"}},refutingEvidence:{type:"array",items:{type:"string"}},unknowns:{type:"array",items:{type:"string"}},noTradeReasons:{type:"array",items:{type:"string"}},sourceRefs:{type:"array",items:{type:"string"}},accountAction:{type:"object",additionalProperties:false,properties:{action:{type:"string",enum:["OPEN","HOLD","CLOSE","REDUCE"]},reason:{type:"string"}},required:["action","reason"]},
  },
  required:["consultationId","expertId","round","skillVersion","snapshotHash","symbol","marketRegime","direction","setupName","contextTimeframe","executionTimeframe","validUntil","triggerConditions","machineTrigger","entryZone","invalidation","stopPrice","targets","managementPlan","leverage","marginUsdt","maxLossUsdt","expectedRr","triggerProbability","winProbabilityGivenTrigger","evidenceCompleteness","supportingEvidence","refutingEvidence","unknowns","noTradeReasons","sourceRefs","accountAction"],
};
...
export async function runExpertRound(input: ExpertRunnerInput, runtime: { provider?: ModelProviderId; env?: ProviderEnv; fetcher?: typeof fetch; target?: CompatibleTarget; targets?: readonly CompatibleTarget[] } = {}): Promise<DecisionContract> {
  ...
  const result = runtime.targets?.length
    ? await invokeStructuredModelWithFallback(request, { provider: runtime.provider, env: runtime.env, fetcher: runtime.fetcher, targets: runtime.targets })
    : await invokeStructuredModel(request, runtime);
