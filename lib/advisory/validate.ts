...
export function validateDecision(value: unknown): ValidationResult {
  ...
  if (item.direction !== "NEUTRAL") {
    ...
    if (!item.machineTrigger || !["PRICE_IN_ZONE", "CLOSE_BREAKOUT", "CLOSE_BREAKDOWN"].includes(item.machineTrigger.type) || !["1h", "4h"].includes(item.machineTrigger.timeframe)) errors.push("directional plan requires a machine trigger");
    if (item.machineTrigger && item.machineTrigger.type !== "PRICE_IN_ZONE" && (typeof item.machineTrigger.price !== "number" || !Number.isFinite(item.machineTrigger.price))) errors.push("close trigger requires a numeric trigger price");
  ...
  if (item.direction === "NEUTRAL" && item.machineTrigger !== null) errors.push("neutral plan machine trigger must be null");
