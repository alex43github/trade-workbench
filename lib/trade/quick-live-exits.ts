...
export function evaluateQuickLiveExit(input: {
  ...
  if (snapshot.exitRule === "BALANCED_MA_1H") {
    const marginProfitLocked = [9, 10, 11].every((offset) => state.completedTargets.includes(offset));
    if (marginProfitLocked) {
      const trailingTriggered = snapshot.templateId === "BALANCED_LONG_1H"
        ? currentCandle.close < snapshot.ma
        : currentCandle.close > snapshot.ma;
      if (!trailingTriggered) return noAction(snapshot, currentCandle.id, "SAFE_CANDLE", state, recordedState);
      const completed = nextState(recordedState, currentCandle.id, { exitCompleted: true });
      return {
        action: "FULL_EXIT", reason: "TRAILING_MA_CLOSE", exitPercent: 100, candleId: currentCandle.id,
