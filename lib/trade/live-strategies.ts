...
export type LiveStrategyExecutionFill = {
  ...
  binanceFillId: string; quantity: string; price: string; executedAt: string;
};

export type LiveStrategyLifecycle = {
  entryQuantity: string; entryVwap: string | null; exitQuantity: string; exitVwap: string | null;
...
function decodeExecutionFill(row: Row): LiveStrategyExecutionFill {
  return {
    ...
    role: String(row.role) as "ENTRY" | "EXIT", binanceFillId: String(row.binance_fill_id),
...
function decodeLifecycle(row: Row | null): LiveStrategyLifecycle {
  return {
    entryQuantity: row?.entry_quantity == null ? "0" : String(row.entry_quantity), entryVwap: textOrNull(row?.entry_vwap),
...
async function backfillLegacyLiveStrategy(strategyId: string) {
  ...
  await db.batch([
    db.prepare(`INSERT OR IGNORE INTO live_strategy_generations
      (id, strategy_id, generation, refresh_reason, status)
      SELECT ?, ?, 1, 'LEGACY_IMPORT', 'ACTIVE'
      WHERE EXISTS (SELECT 1 FROM live_strategy_orders WHERE strategy_id = ?)`)
      .bind(generationId, strategyId, strategyId),
    db.prepare(`INSERT OR IGNORE INTO live_strategy_lifecycle (strategy_id) VALUES (?)`).bind(strategyId),
  ]);
  await db.prepare(`INSERT OR IGNORE INTO live_strategy_order_attempts
    ...
    SELECT 'TW-L-ATTEMPT-LEGACY-' || o.id, o.strategy_id, ?, 1, o.leg_id, o.id, o.intent, o.client_order_id,
    ...
    FROM live_strategy_orders o WHERE o.strategy_id = ?`).bind(generationId, strategyId).run();
}
...
function decodeProtectionLink(row: Row): LiveEntryProtectionLink {
  ...
  return {
    id: String(row.id), liveOrderId: String(row.live_order_id), sourceFillId: String(row.source_fill_id), quantity: String(row.quantity),
...
async function strategyRow(strategyId: string) {
  const row = await (await getD1()).prepare("SELECT id FROM live_strategies WHERE id = ? LIMIT 1").bind(strategyId).first<Row>();
...
export async function ensureLiveStrategyGeneration(input: {
  ...
  await db.prepare(`INSERT OR IGNORE INTO live_strategy_generations
    (id, strategy_id, generation, anchor_candle_id, ma_value, atr_value, next_refresh_at, refresh_reason, status)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    .bind(`TW-L-GEN-${crypto.randomUUID()}`, strategyId, generation, anchorCandleId, maValue, atrValue, nextRefreshAt, refreshReason, status).run();
  await db.prepare("INSERT OR IGNORE INTO live_strategy_lifecycle (strategy_id) VALUES (?)").bind(strategyId).run();
...
export async function createLiveOrderAttempt(input: {
  ...
  const attempt: LiveStrategyOrderAttempt = {
    ...
    legacyLiveOrderId: null, intent: input.intent, clientOrderId, exchangeOrderId: null, side: input.side ?? null,
  ...
  try {
    await db.prepare(`INSERT INTO live_strategy_order_attempts
      (id, strategy_id, generation_id, generation, leg_id, intent, client_order_id, side, type, time_in_force, price, original_quantity, status)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'RESERVED')`)
      .bind(attempt.id, attempt.strategyId, attempt.generationId, attempt.generation, attempt.legId, attempt.intent,
...
export async function recordLiveExecutionFill(input: {
  ...
  await db.prepare(`INSERT OR IGNORE INTO live_strategy_execution_fills
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
    .bind(`TW-L-FILL-${crypto.randomUUID()}`, strategyId, orderAttemptId, input.role, binanceFillId, quantity, price, executedAt).run();
  if (input.role === "ENTRY") {
  ...
  const row = await db.prepare("SELECT * FROM live_strategy_execution_fills WHERE binance_fill_id = ? LIMIT 1").bind(binanceFillId).first<Row>();
...
export async function freezeLiveStrategyEntries(id: unknown, reason: unknown) {
  ...
  if (!lifecycle?.entry_freeze_reason) {
    await db.batch([
      ...
      db.prepare("INSERT INTO live_strategy_events (id, strategy_id, type, payload_json) VALUES (?, ?, 'ENTRY_FROZEN', ?)")
        .bind(crypto.randomUUID(), strategyId, JSON.stringify({ reason: freezeReason, frozenAt: new Date().toISOString() })),
    ]);
  }
  return liveStrategyLifecycle(strategyId);
}
...
export async function recordLiveEntryProtectionLink(input: {
  ...
  await db.prepare(`INSERT INTO live_entry_protection_links
      (id, live_order_id, source_fill_id, quantity, protection_strategy_id, status, error)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    ...
    .bind(`TW-L-P-${crypto.randomUUID()}`, liveOrderId, sourceFillId, String(quantity), protectionStrategyId, status, safeError(input.error)).run();
