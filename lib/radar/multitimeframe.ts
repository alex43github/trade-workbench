...
async function scanSymbol(symbol: string, now: Date, fetchers: MultiTimeframeFetchers) {
  ...
  const insufficientIntervals: MultiTimeframeInterval[] = [];
  for (const interval of intervals) {
    try {
      ...
      if (bars.length < 676) insufficientIntervals.push(interval);
    } catch {
      failedIntervals.push(interval);
    }
  }
  return { byInterval, failedIntervals, insufficientIntervals };
}
...
export async function buildMultiTimeframeSnapshot(
  ...
  async function worker() {
    while (cursor < symbols.length) {
      ...
      scannedSymbols += 1;
      if (VEGAS_INTERVALS.some((interval) => result.byInterval[interval]?.vegasAligned)) matchedSymbols += 1;
  ...
  for (const symbol of symbols) {
    ...
    insufficientCount += result.insufficientIntervals.length;
    for (const interval of VEGAS_INTERVALS) {
      if (result.byInterval[interval]?.vegasAligned) vegas[interval].push(symbol);
    }
  }
  ...
  if (insufficientCount) warnings.push("部分周期历史不足 676 根，无法计算完整 Vegas 通道");
  const progress = createScanProgress(symbols.length, symbols.length, matchedSymbols);
  await options.onProgress?.(progress);
  return {
    status: symbols.length > 0 && failedSymbols === 0 && failedIntervals === 0 && insufficientCount === 0 ? "ready" : "degraded",
