...
async function readSymbolLeverage(symbol: string, dependencies: TelegramHandlerDependencies): Promise<LiveSymbolLeverage> {
  const read = dependencies.getLiveSymbolLeverage ?? getLiveSymbolLeverage;
  try {
    return await read(symbol);
  } catch {
  ...
  if (update.kind === "CALLBACK") {
    ...
    if (actionId === "quick_order_01") {
      await transitionConversation(update.userId, dependencies, {
        ...
        step: "QUICK_SYMBOL", confirmNonce: null,
      }, { reset: true });
      return screen("默认快速下单。请输入币种，例如 BTCUSDT 或 BTCUSDC。", cancelAndBackKeyboard);
    }
    ...
    if (actionId === "confirm_protection_01") {
      ...
      try {
        const latest = await findPositions();
    ...
    if (actionId === "quick_side_long_01" || actionId === "quick_side_short_01") {
      ...
      return screen("已记录方向。请选择 K 线周期：15分钟、1小时、4小时或日线。", withBack(quickTimeframeKeyboard));
    }
    const quickTimeframes: Record<string, string> = { quick_tf_15m_01: "15m", quick_tf_1h_01: "1h", quick_tf_4h_01: "4h", quick_tf_1d_01: "1d" };
  ...
  if (update.kind === "MESSAGE") {
    ...
    if (session.step === "QUICK_SYMBOL") {
      try {
        ...
        const leverage = await readSymbolLeverage(symbol, dependencies);
        await transitionConversation(update.userId, dependencies, {
          symbol,
